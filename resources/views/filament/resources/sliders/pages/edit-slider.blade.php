<x-filament-panels::page>
    ...
    <form action="{{ route('admin.sliders.update', $this->record) }}" method="POST" enctype="multipart/form-data" class="custom-filament-form">
        @csrf
        @method('PUT')
        ...
        <div class="custom-form-card">
            <div class="custom-form-grid">
                <div class="custom-field">
                    ...
                    <p class="custom-help">This text appears as the main slider heading.</p>
                </div>

                <div class="custom-field">
                    <label for="subtitle">Slider Subtitle</label>
                    <input type="text" id="subtitle" name="subtitle" value="{{ old('subtitle', $this->record->subtitle) }}" required>
                </div>

                <div class="custom-field">
                    <label>Current Image</label>
                    @if ($this->record->image_path)
                        <img src="{{ route('sliders.image', $this->record) }}?v={{ optional($this->record->updated_at)->timestamp }}" alt="{{ $this->record->title }}" class="custom-preview">
                    @else
                        <p class="custom-help">No image has been uploaded.</p>
                    @endif
                </div>

                <div class="custom-field">
                    <label for="image">Replace Slider Image</label>
                    <input type="file" id="image" name="image" accept="image/jpeg,image/png,image/webp">
                    <p class="custom-help">Leave empty if you do not want to replace the image. Maximum file size: 8 MB.</p>
                </div>

                <div class="custom-field">
                    <label for="sort_order">Sort Order</label>
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $this->record->sort_order) }}" min="0">
                </div>

                <div class="custom-field">
                    <label for="duration_ms">Slide Duration (milliseconds)</label>
                    <input type="number" id="duration_ms" name="duration_ms" value="{{ old('duration_ms', $this->record->duration_ms ?? 3000) }}" min="1000" max="30000" step="100">
                    <p class="custom-help">Example: 3000 means the slide changes every 3 seconds.</p>
                </div>
        ...
        <div class="custom-actions">
            ...
            <a href="{{ \App\Filament\Resources\Sliders\SliderResource::getUrl('index') }}" class="custom-btn-secondary">Cancel</a>
        </div>
    </form>
</x-filament-panels::page>
