...
<style>
    ...
    .hero .hero-slide::after {
        ...
        position: absolute;
        inset: 0;
        background: linear-gradient(90deg, rgba(7, 43, 87, .34), rgba(7, 43, 87, .20), rgba(7, 43, 87, .44)) !important;
        pointer-events: none
    }
    ...
    @media(max-width:768px) {
        ...
        .section-header:has(#apiNewsPagination) .section-title {
        ...
        .pagination#apiNewsPagination::-webkit-scrollbar {
            display: none !important
        }
        ...
        .page-jump {
            ...
            border-left: 1px solid rgba(7, 43, 87, .12) !important;
    ...
    <div class="container">
        <div class="footer-content">
            <div class="footer-column">
                ...
                <div class="footer-item"><i class="fas fa-globe"></i><a href="https://www.unw.ac.id"
                        target="_blank">universitasngudiwaluyo.com</a></div>
            </div>
            <div class="footer-column">
                <h3>LOKASI</h3>
                <div class="map-container"><iframe class="footer-map"
            ...
            <div class="footer-column">
                ...
                <div class="social-icons"><a href="#"><i class="fab fa-facebook"></i></a><a href="#"><i
                            class="fab fa-instagram"></i></a><a href="#"><i class="fab fa-x-twitter"></i></a><a
    ...
    <script>
        (function() {
            ...
            const hero = document.querySelector('.hero'),
                dotsWrapper = document.getElementById('heroDots'),
                titleEl = document.querySelector('.hero-title'),
                subtitleEl = document.querySelector('.hero-subtitle'),
            ...
            function goToSlide(nextIndex) {
                ...
                const total = slides.length,
                    targetIndex = (nextIndex + total) % total;
                if (targetIndex === currentIndex) return;
                isAnimating = true;
                const currentSlide = slides[currentIndex],
                    nextSlide = slides[targetIndex];
    ...
    (function() {
        ...
        function readMeta(payload) {
            ...
            return {
                ...
                perPage: Number(meta.per_page || 4)
            }
        }
        ...
        function renderFilters(section, state) {
            ...
            if (!target) return;
            let html =
                '<button class="news-filter-btn active" type="button" data-id="all" data-slug="all">Semua</button>';
            html += state.categories.map(function(category) {
                return '<button class="news-filter-btn" type="button" data-id="' + esc(category.id) +
                    '" data-slug="' + esc(category.slug) + '">' + esc(category.name) + '</button>'
            }).join('');
            target.innerHTML = html;
            target.querySelectorAll('.news-filter-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    target.querySelectorAll('.news-filter-btn').forEach(function(item) {
        ...
        function renderNews(section, items) {
            ...
            target.innerHTML = items.map(function(news) {
                ...
                return '<article class="news-item"><a class="news-item-link" href="' + esc(newsDetailUrl(
                    ...
                    '</div><div class="news-content"><div class="news-category"><svg viewBox="0 0 24 24"><path d="M20.59 13.41 12 22l-8.59-8.59A2 2 0 0 1 3 12V4a1 1 0 0 1 1-1h8a2 2 0 0 1 1.41.59L22 12.17l-1.41 1.24ZM7.5 8A1.5 1.5 0 1 0 7.5 5 1.5 1.5 0 0 0 7.5 8Z"/></svg>' +
                    esc(news.categoryName) + '</div><h3 class="news-title">' + esc(news.title) + '</h3>' + (
                        excerpt ? '<p class="news-excerpt">' + esc(excerpt.slice(0, 155)) + '</p>' : '') +
                    '<div class="news-date"><svg viewBox="0 0 24 24"><path d="M7 2h2v2h6V2h2v2h3a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1h3V2Zm12 8H5v10h14V10ZM5 8h14V6H5v2Zm2 4h3v3H7v-3Zm5 0h3v3h-3v-3Z"/></svg>' +
                    esc(formatDate(news.date)) + '</div></div></a></article>'
            }).join('')
        }
        ...
        function renderPagination(section, state) {
            ...
            let html = '<button class="page-btn" type="button" data-page="' + (current - 1) + '" ' + (current <= 1 ?
                'disabled' : '') + '>‹</button>';
            for (let page = start; page <= end; page++) {
                html += '<button class="page-number ' + (page === current ? 'active' : '') +
                    '" type="button" data-page="' + page + '">' + page + '</button>'
            }
            if (end < last) {
                html += '<span>...</span><button class="page-number" type="button" data-page="' + last + '">' +
                    last + '</button>'
            }
            html += '<button class="page-btn dark" type="button" data-page="' + (current + 1) + '" ' + (current >=
                    last ? 'disabled' : '') +
                '>›</button><div class="page-jump"><input type="number" min="1" max="' + last + '" value="' +
                current + '" aria-label="Pilih halaman"><button type="button">Go</button></div>';
            target.innerHTML = html;
            ...
            if (jumpInput) jumpInput.addEventListener('keydown', function(event) {
        ...
        async function loadNews(section, state) {
            ...
            try {
                ...
                let items = toArray(payload).map(normalizeNews);
                const meta = readMeta(payload);
                if (category) {
                    const before = items.length;
                    items = items.filter(function(news) {
                        return String(news.categoryId) === String(category.id) || String(news
                            .categorySlug) === String(category.slug)
                    });
                    if (!items.length && before) {
                        const largePayload = await getJson(API.news + '?paginate=100&page=1');
                        items = toArray(largePayload).map(normalizeNews).filter(function(news) {
                            return String(news.categoryId) === String(category.id) || String(news
                                .categorySlug) === String(category.slug)
                        });
                        state.lastPage = Math.max(1, Math.ceil(items.length / 4));
                        items = items.slice((state.page - 1) * 4, state.page * 4)
                    } else {
                ...
                state.page = meta.current && (!category || items.length) ? meta.current : state.page;
                renderNews(section, items);
                renderPagination(section, state)
            } catch (error) {
                if (target) target.innerHTML =
                    '<div class="news-state">Berita belum dapat dimuat. Silakan periksa koneksi internet Anda.</div>'
            }
        }
        async function initInfoSectionApi() {
            ...
            if (!newsArea) return;
            newsArea.innerHTML =
                '<div class="section-header"><h2 class="section-title">Berita Terkini & Agenda</h2><div class="pagination" id="apiNewsPagination"></div></div><div class="news-filter-bar" id="apiCategoryFilter"></div><div class="news-list" id="apiNewsList"><div class="news-loading"><div class="news-loader"></div></div></div>';
            const state = {
                page: 1,
                lastPage: 1,
                category: {
            ...
            try {
                ...
                state.categories = toArray(categoryPayload).map(normalizeCategory).filter(function(category) {
        ...
        function isUsableRouteLink(link) {
            ...
            if (!trimmedHref || trimmedHref === '#' || trimmedHref.startsWith('#') || trimmedHref.toLowerCase()
        ...
        function activateHeaderByCurrentPage() {
            ...
            const navItems = navMenu.querySelectorAll('.nav-item'),
                ...
                dropdownLinks = navMenu.querySelectorAll('.dropdown a'),
                homeNavItem = document.getElementById('homeNavItem');
            ...
            dropdownLinks.forEach(link => {
                ...
                if (linkPath === currentPath) activeDropdownLink = link
            });
            if (activeDropdownLink) {
                const parentItem = activeDropdownLink.closest('.nav-item'),
                    parentLink = parentItem?.querySelector(':scope > .nav-link');
            ...
            navLinks.forEach(link => {
                if (!isUsableRouteLink(link)) return;
                const linkPath = normalizePath(link.href);
                if (linkPath === currentPath) activeTopLink = link
            });
            if (activeTopLink) {
                const parentItem = activeTopLink.closest('.nav-item');
                parentItem?.classList.add('route-active');
                activeTopLink.classList.add('nav-route-active');
                if (parentItem?.id === 'homeNavItem') homeNavItem?.classList.remove('hide-indicator');
                else homeNavItem?.classList.add('hide-indicator');
                return
            }
            if (currentPath.startsWith('/akademik')) {
                const akademikItem = Array.from(navItems).find(item => item.textContent.trim().toLowerCase()
                        .startsWith('akademik')),
                    akademikLink = akademikItem?.querySelector(':scope > .nav-link');
                akademikItem?.classList.add('route-active');
                akademikLink?.classList.add('nav-route-active');
                homeNavItem?.classList.add('hide-indicator');
                return
            }
            if (currentPath.startsWith('/profil') || currentPath === '/visi-misi' || currentPath ===
                '/tentang-pascasarjana') {
                const profilItem = Array.from(navItems).find(item => item.textContent.trim().toLowerCase()
                        .startsWith('profil')),
                    profilLink = profilItem?.querySelector(':scope > .nav-link');
        ...
        function bindProgramDetailLinks() {
            const programCards = document.querySelectorAll('.program-section .program-card'),
                programDetailRoutes = ["{{ route('akademik.show', 'magister-hukum') }}",
