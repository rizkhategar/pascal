    ...
    (function() {
        ...
        function renderFilters(section, state) {
            ...
            html += state.categories.map(category => '<button class="news-filter-btn" type="button" data-id="' +
                esc(category.id) + '" data-slug="' + esc(category.slug) + '">' + esc(category.name) +
                '</button>').join('');
        ...
        function renderNews(section, items) {
            ...
            target.innerHTML = items.map(news => {
                ...
                return '<article class="news-item"><a class="news-item-link" href="' + esc(newsDetailUrl(
                    ...
                    '</div><div class="news-content"><div class="news-category">' + esc(news.categoryName) +
                    '</div><h3 class="news-title">' + esc(news.title) + '</h3>' + (excerpt ?
                        '<p class="news-excerpt">' + esc(excerpt.slice(0, 155)) + '</p>' : '') +
                    '<div class="news-date">' + esc(formatDate(news.date)) + '</div></div></a></article>'
            }).join('')
        }
        ...
        function renderPagination(section, state) {
            ...
            let start = Math.max(1, current - 1),
                end = Math.min(last, start + 3);
            if (end - start < 3) start = Math.max(1, end - 3);
            let html = '<button class="page-btn" type="button" data-page="' + (current - 1) + '" ' + (current <= 1 ?
                'disabled' : '') + '>‹</button>';
            for (let page = start; page <= end; page++) html += '<button class="page-number ' + (page === current ?
                'active' : '') + '" type="button" data-page="' + page + '">' + page + '</button>';
            if (end < last) html += '<span>...</span><button class="page-number" type="button" data-page="' + last +
                '">' + last + '</button>';
            ...
            target.innerHTML = html;
            target.querySelectorAll('button[data-page]').forEach(button => button.addEventListener('click',
                function() {
            ...
            const jumpInput = target.querySelector('.page-jump input'),
                jumpButton = target.querySelector('.page-jump button');
