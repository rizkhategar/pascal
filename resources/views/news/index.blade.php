...
<body>
    ...
    <script>
        (function(){
            ...
            function render(items){const q=state.q.toLowerCase();if(q)items=items.filter(n=>(n.title+' '+strip(n.excerpt)).toLowerCase().includes(q));if(!items.length){grid.innerHTML='<div class="empty">Belum ada berita yang sesuai.</div>';return}grid.innerHTML=items.map(n=>'<a class="news-page-card" href="/berita/'+encodeURIComponent(n.slug)+'"><div class="news-page-thumb">'+(n.image?'<img src="'+esc(img(n.image))+'" alt="'+esc(n.title)+'">':'')+'</div><div class="news-page-body"><div class="news-page-category"><i class="fas fa-tag"></i>'+esc(n.categoryName)+'</div><h2 class="news-page-title">'+esc(n.title)+'</h2><p class="news-page-excerpt">'+esc(strip(n.excerpt))+'</p><div class="news-page-date"><i class="fas fa-calendar-alt"></i>'+esc(date(n.date))+'</div></div></a>').join('')}
