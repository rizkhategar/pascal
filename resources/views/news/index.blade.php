...
<head>
    ...
    <style>
        ...
        * {
        ...
        .news-hero {
            background: linear-gradient(135deg, #052044, #083b73);
            color: #fff;
            padding: 72px 0;
            position: relative;
            overflow: hidden
...
<body>
    ...
    <section class="news-hero">
        <div class="container">
            ...
            <p class="page-desc">Kumpulan berita, agenda, dan informasi terbaru Pascasarjana Universitas Ngudi Waluyo.
            </p>
        </div>
    </section>
    <main class="news-section">
        <div class="container">
            <div class="news-toolbar">
                <div class="filter-bar" id="newsFilters"><button class="filter-btn active"><i
                            class="fas fa-tag"></i>Semua</button></div>
                <div class="news-search-wrap"><input class="search-box" id="newsSearch" type="search"
                        placeholder="Cari berita..."><button class="search-icon-btn" id="newsSearchButton"
                        type="button" aria-label="Cari berita"><i class="fas fa-magnifying-glass"></i></button></div>
            </div>
    ...
    <script>
        (function(){
            ...
            // Ubah agar API berita SELALU melewati route NewsController internal Anda (route('news.search'))
            ...
            // Menggabungkan URL builder agar SELALU menggunakan backend cache Anda
            function buildUrl(page){
                const p=new URLSearchParams({paginate:String(PAGE_SIZE),page:String(page)});
                if(state.q.trim() !== '') p.set('q', state.q.trim());
                if(state.category.id !== 'all') { p.set('category_id', state.category.id); p.set('category', state.category.slug||state.category.id); }
                return API.search+'?'+p.toString();
            }
            
            function render(items){if(!items.length){grid.innerHTML='<div class="empty">Belum ada berita yang sesuai dengan pencarian Anda.</div>';return}grid.innerHTML=items.map(n=>'<a class="news-page-card" href="/berita/'+encodeURIComponent(n.slug)+'"><div class="news-page-thumb">'+(n.image?'<img src="'+esc(img(n.image))+'" alt="'+esc(n.title)+'">':'')+'</div><div class="news-page-body"><div class="news-page-category"><i class="fas fa-tag"></i>'+esc(n.categoryName)+'</div><h2 class="news-page-title">'+esc(n.title)+'</h2><p class="news-page-excerpt">'+esc(strip(n.excerpt))+'</p><div class="news-page-date"><i class="fas fa-calendar-alt"></i>'+esc(date(n.date))+'</div></div></a>').join('')}
            ...
            async function load(){
                ...
                grid.innerHTML='<div class="loading"><div class="loader"></div></div>';
                try{
                    ...
                    if(requestId===activeRequestId)grid.innerHTML='<div class="empty">Berita belum dapat dimuat. Coba muat ulang halaman.</div>';
                }
            }
            ...
            search.addEventListener('input',()=>{clearTimeout(searchTimer);state.q=search.value;state.page=1;searchTimer=setTimeout(load,400)});
