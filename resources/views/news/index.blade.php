...
<body>
    ...
    <script>
        (function(){
            ...
            function renderPages(){let html='';const last=Math.max(1,state.lastPage),cur=Math.min(state.page,last);for(let p=Math.max(1,cur-2);p<=Math.min(last,cur+2);p++)html+='<button class="page-btn '+(p===cur?'active':'')+'" data-page="'+p+'">'+p+'</button>';pagination.innerHTML='<button class="page-btn" data-page="'+(cur-1)+'" '+(cur<=1?'disabled':'')+'>‹</button>'+html+'<button class="page-btn" data-page="'+(cur+1)+'" '+(cur>=last?'disabled':'')+'>›</button>';pagination.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{const p=Number(b.dataset.page);if(p>=1&&p<=last&&p!==cur){state.page=p;load()}})}
            ...
            search.addEventListener('input',()=>{state.q=search.value;load()});
            loadFilters();load();
        })();
    </script>
</body>
