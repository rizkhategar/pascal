...
<body>
    ...
    <script>
        (function(){
            ...
            function renderNews(news){const title=news?.title||'Detail Berita';const category=(news?.category?.name||'Berita').trim();const image=news?.image||news?.image_thumbnail||'';const excerpt=news?.excerpt||'';const body=news?.content||news?.body||news?.description||'';const date=news?.publishedAt||news?.published_at||news?.createdAt||news?.created_at||'';const author=news?.author?.name||'';const views=news?.views;document.title=title+' - Pascasarjana UNW';document.getElementById('newsCategory').textContent=category;document.getElementById('newsTitle').textContent=title;const meta=[];if(date)meta.push('<span>'+esc(formatDate(date))+'</span>');if(author)meta.push('<span>Ditulis oleh '+esc(author)+'</span>');if(views!==undefined&&views!==null)meta.push('<span>'+esc(views)+' dibaca</span>');document.getElementById('newsMeta').innerHTML=meta.join('');const imageHtml=image?'<div class="news-cover-wrap"><img src="'+esc(image)+'" alt="'+esc(title)+'" class="news-cover"></div>':'';const excerptHtml=excerpt?'<p class="news-excerpt-detail">'+esc(excerpt)+'</p>':'';const bodyHtml=body?body:'<p>'+esc(excerpt||'Isi lengkap berita belum tersedia dari API. Data yang tersedia saat ini berasal dari ringkasan berita berdasarkan slug.')+'</p><a class="external-link" href="'+API_ORIGIN+'/news/'+esc(slug)+'" target="_blank" rel="noopener">Buka sumber berita asli →</a>';document.getElementById('newsCard').innerHTML=imageHtml+'<div class="news-body">'+excerptHtml+'<div class="news-content-html">'+bodyHtml+'</div></div>'}
